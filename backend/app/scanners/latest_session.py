...
markets closed): ranks the universe by the most recently completed trading
session's real close-to-close % change, computed once at startup, instead
of showing an empty scanner or fabricating data.
"""
...
async def compute_latest_session_gainers(
    ...
    for i in range(0, len(symbols), _BATCH_SIZE):
        ...
        try:
            ...
            logger.exception(
                "Latest-session gainers fetch failed for a batch of %d symbols", len(batch)
            )
            continue
