...
A candidate may also carry merger_activity: a structured, Alpaca-confirmed corporate-action record (acquirer, deal status, and per-share cash terms when the deal is cash) -- present only when Alpaca has an actual filed merger action on this symbol, never inferred from the headline's wording. When present, treat it as a hard, verified confirmation of an M&A catalyst rather than another thing to weigh against the headline text; when absent, an M&A-sounding headline is still just a headline, not a confirmed corporate action, so describe it as reported news rather than as a done deal.
...
def _row_to_payload(
    ...
    return {
        ...
        # Structured, Alpaca-confirmed corporate action -- not inferred from
        # headline wording. See _SYSTEM_PROMPT for how this should be read
        # relative to recent_headline's own possibly-M&A-flavored text.
        "merger_activity": (
            {
                "acquirer_symbol": row.merger_action.acquirer_symbol,
                "status": row.merger_action.sub_type,
                "cash_consideration_per_share": row.merger_action.cash_consideration,
            }
            if row.merger_action
            else None
        ),
    }
